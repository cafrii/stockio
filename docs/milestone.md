...
## Phase 2: 증권사 API 이중화 및 금 시세 (신규)

> 본 Phase의 상세 계획·진행 상태는 별도 문서로 관리한다.
> - 상세 마일스톤: `docs/phase2/milestone.md`
...
### Phase 4.1: 추가 데이터 항목 지원
- [ ] 시고저종, 등락률, 거래량 등 추가
- [ ] XML 스키마 확장

### Phase 4.2: 다른 자산 클래스 지원
- [ ] 암호화폐 (비트코인, 이더리움)
- [ ] (KRX 금 현물 시세는 Phase 2.2로 이관)
...
2. **E2E 시험** (Phase 1.3, Phase 3.3)
   - Phase 1.3: 로컬 환경에서 기본 동작 확인
   - Phase 3.3: 프로덕션 환경에서 종합 검증

3. **회귀 테스트**
   - Phase 3 이후 변경 사항 발생 시, 핵심 시나리오만 재테스트
